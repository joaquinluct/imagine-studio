...
// Esta funci�n est� implementada en la biblioteca ImGui que linkamos
extern LRESULT ImGui_ImplWin32_WndProcHandler(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam);

namespace Platform {

// ?? BUG-002 Intento #6: Definir map est�tico fuera de la clase
    ...
    : hInstance_(hInstance)
{
    ...
    // ? Verificar si la clase ya est� registrada
    ...
    else
    {
        ...
        if (atom == 0)
        {
            ...
            // ? CR�TICO: Abortar si no podemos registrar la clase
    ...
    // Registraremos la asociaci�n HWND ? Window* manualmente despu�s de creaci�n
    hwnd_ = CreateWindowExW(
        ...
        nullptr  // ?? NO pasar 'this' aqu� (evitar problemas con WM_NCCREATE)
    );
    ...
    // ?? BUG-002 Intento #6: Registrar asociaci�n HWND ? Window* en map est�tico
...
Window::~Window()
{
    // ?? BUG-002 Intento #6: Eliminar asociaci�n del map est�tico
    if (hwnd_)
    {
...
{
    // ?? BUG-002 Intento #6: Buscar Window* en map est�tico en lugar de GWLP_USERDATA
...
LRESULT Window::WndProc(UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    // ? CR�TICO: Pasar eventos a ImGui PRIMERO y respetar su retorno (v1.3.0 - H2.3)
    // Si ImGui captura el evento (retorna != 0), NO debemos procesarlo nosotros
    // Esto es ESENCIAL para que los clicks de rat�n funcionen en la UI
