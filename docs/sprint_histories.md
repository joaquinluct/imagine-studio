...
**Para** tener una base sólida de Editor UI siguiendo estándares AAA

**Prioridad**: Alta  
**Estimación**: 5 Story Points

**Descripción detallada**:
Esta historia cubre la integración inicial de ImGui en el proyecto: descargar la librería, añadirla al sistema de build (CMake + Visual Studio), crear el context de ImGui, y configurar estilo/theme por defecto. Al finalizar, ImGui debe compilar sin errores y estar listo para renderizar UI.

**Criterios de Aceptación**:
- [ ] ImGui descargado e integrado en el proyecto (carpeta `external/imgui/`)
- [ ] CMakeLists.txt actualizado con ImGui como library estática
...
- [ ] Compilación limpia (CMake Debug + MSBuild Debug, 0 errores, 0 warnings)
...
**Para** poder renderizar UI panels con text rendering y geometría ImGui

**Prioridad**: Alta  
**Estimación**: 8 Story Points

**Descripción detallada**:
Esta historia cubre la integración del backend oficial `imgui_impl_dx12.cpp` para renderizar ImGui con DirectX 12. Incluye crear descriptor heap para ImGui (SRV para font atlas), configurar command list, y renderizar ImGui draw data en `UIPass()`. Al finalizar, ImGui debe renderizar text y widgets correctamente.
...
**Descripción detallada**:
Esta historia cubre la creación de 4 panels principales de Editor (Hierarchy, Inspector, Console, Viewport) y habilitar docking/multi-window. Incluye crear funciones de rendering para cada panel, habilitar `ImGuiConfigFlags_DockingEnable`, y renderizar viewport con render target del renderer. Al finalizar, el Editor debe tener 4 panels flotantes con docking funcional.

**Criterios de Aceptación**:
- [ ] Panel "Hierarchy" creado (árbol de objetos placeholder)
