...
msbuild "build/ImagineStudio.sln" /t:Build /p:Configuration=Debug /p:Platform=x64 /v:q

# ✅ Option 2: Minimal output (only errors/warnings)
msbuild "build/ImagineStudio.sln" /t:Build /p:Configuration=Debug /p:Platform=x64 /v:minimal

# ✅ Option 3: Normal output with Out-String (if logs needed)
msbuild "build/ImagineStudio.sln" /t:Build /p:Configuration=Debug /p:Platform=x64 2>&1 | Out-String

# ❌ NEVER: Filter with Select-Object
msbuild ... 2>&1 | Select-Object -Last 5  # BLOCKS CHAT

# ⚠️ IMPORTANT: Always use build/ImagineStudio.sln (CMake-generated)
# DO NOT use "Imagine Studio.sln" in root (obsolete)
```
...
```powershell
# IMPORTANTE: CMake genera archivos de proyecto en build/
# El usuario debe abrir build/ImagineStudio.sln en Visual Studio para trabajar
# F5 y Ctrl+Shift+B deben ejecutarse sobre build/ImagineStudio.sln

# Comando de validación MSBuild (desde línea de comandos):
msbuild "build/ImagineStudio.sln" /t:Build /p:Configuration=Debug /p:Platform=x64 /m
```

**Why**: This is what user presses with **Ctrl+Shift+B** or **F5** in Visual Studio. If this fails, user **CANNOT WORK**.

**CRITICAL**: 
- ✅ **CORRECT solution file**: `build/ImagineStudio.sln` (generated by CMake)
- ❌ **DO NOT USE**: `Imagine Studio.sln` (root) - obsolete, references deleted `.vcxproj`
- ✅ **User workflow**: Open `build/ImagineStudio.sln` in Visual Studio → Press F5 or Ctrl+Shift+B
